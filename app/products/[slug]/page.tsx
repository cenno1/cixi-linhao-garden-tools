...
export default async function ProductDetailPage({ params }: Props) {
  ...
  const productImageUrls = productImages.map(({ src }) => new URL(src, siteUrl).toString());
  const related = products.filter((item) => item.category === product.category && item.slug !== product.slug).slice(0, 3);
  const productSchema = { "@context": "https://schema.org", "@type": "Product", name: product.name, description: product.summary, image: productImageUrls, sku: product.code, category: product.category, brand: { "@type": "Brand", name: "CIXI LINHAO" }, url: productUrl, additionalProperty: product.features.map((feature) => ({ "@type": "PropertyValue", name: "Key feature", value: feature })) };
  ...
  return <>
    ...
    <main>
      ...
      <section className="section"><div className="container product-detail-grid"><div className="product-detail-gallery">{productImages.map((image, index) => <div className={`product-detail-image${index === 0 ? "" : " product-detail-image--secondary"}`} key={image.src}><img src={image.src} alt={image.alt} /></div>)}</div><div className="product-detail-copy"><span className="product-code">{product.code}</span><h2>Product overview</h2><p>{product.summary} Confirm the reel-side thread, hose-side thread, seal type, installation clearance and required quantity so our team can review the correct configuration before quotation.</p><div className="product-detail-columns"><div><h3>Key features</h3><ul>{product.features.map((feature) => <li key={feature}>{feature}</li>)}</ul></div><div><h3>Typical applications</h3><ul>{product.applications.map((application) => <li key={application}>{application}</li>)}</ul></div></div><a className="button" href={`/contact?product=${encodeURIComponent(`${product.code} ${product.name}`)}`}>Request specifications</a></div></div></section>
