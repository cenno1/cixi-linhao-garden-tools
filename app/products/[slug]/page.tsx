...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (!product) return {};
  const description = product.seoDescription || (product.brassCategory
  ...
  const socialTitle = product.seoTitle || `${product.name} | ${product.code}`;
  return {
    title: product.seoTitle || (product.brassCategory ? `${product.name} Manufacturer | ${product.code}` : `${product.name} | ${product.code}`),
    description,
...
export default async function ProductDetailPage({ params }: Props) {
  ...
  return <>
    ...
    <main>
      ...
      {engineeringSpecs.length > 0 ? <section className="section product-customization"><div className="container product-customization-grid"><div><span className="eyebrow eyebrow-light">Customization options</span><h2>Need a different size or thread?</h2><p>Share the target application, mating components and technical requirements. Each option is reviewed against the approved drawing, sample and intended market before quotation.</p></div><div className="customization-panel"><p className="customization-panel-title">We can customize:</p><ul>{customizationOptions.map((option, index) => <li key={option}><span>{String(index + 1).padStart(2, "0")}</span>{option}</li>)}</ul><a className="customization-upload-button" href={`/contact?product=${encodeURIComponent(`${product.code} ${product.name}`)}#quote-form`}><span>Upload Your Drawing</span><small>PNG, JPG, WEBP or PDF · Max 10 MB</small></a></div></div></section> : null}
